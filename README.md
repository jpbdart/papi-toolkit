...
**PAPI** stands for Provenance-Aware Parse Insertion for LangSec Mediation. The initial release is a proof-of-concept taint analyzer which demonstrates provenance-aware parsing; pass-through parameters in a routine do not need to be parsed, only those that changed.
...
**Note:** We currently are testing the analyzer before we move it to the "production" branch, and here. As soon as we complete that, we will finish uploading code to the repository.
